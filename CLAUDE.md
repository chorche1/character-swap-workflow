...
**🔁 Repurpose auto-send is a CHOICE (2026-08-09, Hugo's directive).** A repurpose used to always deliver, on all three surfaces, with no way to say no. The shared Repurpose modal now carries one "➤ Skicka automatiskt till Telegram" ✓ read by every surface: Swap Step-6 (`runner_compile._compile_one_character`, per character, `auto_telegram_send` param gating the `_REPURPOSE_SLOT` send — the compile slot never sent and still doesn't), the Reengineer/Swap run card (`runner_reengineer.repurpose` → `_repurpose_auto_send(state)` gating `auto_finalize.send_reengineer_repurposed`), and a saved Editor reel (`repurpose_editor_job`). **TICKED by default** — today's behavior is preserved and an unticked box is the new, opt-in thing; a MISSING stored value therefore resolves to True everywhere (in `_repurpose_auto_send`, in the three request bodies' defaults, and in `_repurposeBody()`, which uses `=== undefined ? true` rather than `!!` so an older cached client seed can't silently stop delivering). Unticking withholds ONLY delivery: the mirrored copy is still built, marked `done`, playable on the card, and the manual ➤ Telegram button still sends it — and the Editor path emits no "Telegram misslyckades" push, because a deliberate skip is not a failure. Remembered PER RUN: the flag rides the existing `repurpose_settings` (run state) / `job.repurpose_settings` / `editor_meta.repurpose.settings`, so re-opening the modal for the same run/job/reel shows the last choice, and `_mapStoredToReAsm` rehydrates the ✓. **`api._store_assemble_settings` deliberately DROPS the key** (like `char_ids`): the run-level `state["auto_telegram_send"]`, chosen at upload, governs the ORIGINAL finals after an assemble, and a same-named key inside `assemble_settings` would read like it governs that — `_repurpose_auto_send` also ignores the run-level flag, since a repurpose is an explicit per-click request. `runner_reengineer._repurpose_settings` filters to `ASSEMBLE_DEFAULTS`, so the delivery flag never leaks into the editor-pipeline config. Locked by `test_repurpose_telegram_toggle.py` (26 cases, each gate verified to FAIL when reverted) + `tests/js/repurpose_telegram_toggle.mjs`.

**🎞 Versions — repurpose with an editable scene list (2026-08-10, Hugo's directive).** "Som repurpose, men jag kan också lägga till, ta bort scener, och duplicera scener." A finished Reengineer / Swap-from-images run can carry any number of NAMED versions, each an alternative CUT built into its own extra video per character, with its own player, download, ➤ Telegram button and 🔁 mirror flag. **The original final is never read or written by any of it.** Architecture is *one asset pool, many cuts*: a version's own scenes are registered as ORDINARY scenes on the underlying job — so image generation, QC, the 👥 person gate, ✎↻ per-clip retake and 📥 import all work on them unchanged — and are marked only by an `owner_version` tag on their `state["scenes"]` entry. **Order and repetition live ONLY in `state["versions"][vid]["rows"]`** (`{row_id, scene_id}`), never in `state["scenes"]`: that list is idx-addressed by a dozen endpoints and re-stamped by `_renumber_scenes` after every mutation, so a second ordering stored there would be rewritten out from under the version — and it means a version reorder never calls the scene-order endpoint, which is what stops a version edit from permuting the ORIGINAL reel. **Version-owned scenes are APPENDED, never inserted**, so base entries keep contiguous indices and the run card's numbering is untouched. New modules: `versions.py` (pure cut helpers — `base_scenes` / `base_cut` / `resolve_rows` / `version_cut`) and `runner_versions.py` (the builder, a clone of `_do_repurpose`'s CONTRACT: own guard `_BUILDING_VERSIONS` keyed (re_id, vid), own bucket, never touches `status`/`finals`/`finals_stale`, Telegram auto-send only AFTER the guard is released).

**`version_cut` STRIPS the owner tag from its copies**, which is the load-bearing trick: it lets `_collect_clips`, `_assembly_gaps` and `_char_is_uninvolved` filter `owner_version` UNCONDITIONALLY — given the run's state they drop every version's scenes, given a cut they drop nothing. A filter each caller must remember to pass is a filter that can be forgotten, and forgetting it puts a version's scene into the original reel. **A row whose scene was deleted is REPORTED, never skipped** (`resolve_rows` returns `dangling`; `runner_versions.preflight` refuses loudly naming the position): both build readers treat an absent scene as "never this character's" and drop it in SILENCE, which is verbatim the 2026-08-06 failure that shipped a final missing a scene with the gate saying `{ok}`. **"Ändrad" is DERIVED, not stored** (`runner_versions.is_stale` compares each resolved clip's `submitted_at` to the version's `built_at`): six paths can replace a clip (↻ retry, ✎↻, 📥 import, scene redo, timeout salvage, post-restart resume) and a flag written by five of them lies on the sixth. Hugo's choice: a stale version gets an "ändrad" badge + ▶ Bygg om — never an automatic rebuild or re-delivery.

**`deliverables.py` is the variant registry** that made a THIRD deliverable possible at all. `variant` was a hardcoded two-value enum in ten places (`api.py` ×9 + `auto_finalize.py:304`), every one reading "not final, therefore repurpose". The dangerous one: `drive_push.drive_final_name` / `telegram_delivery.character_final_name` suffixed only on `"repurpose"`, so a third variant produced a filename BYTE-IDENTICAL to the original final's and `google_drive.upload_or_replace` keys on the name — it would have overwritten the original. `deliverables.name_suffix` guarantees a distinct name per variant and falls back to the version id when no label is passed, so the collision is impossible even from a forgetful caller; `final` and `repurpose` names are byte-identical to before, so existing Drive files keep their overwrite key. Also `char_entries` (bucket resolver, writes through for receipts), `is_building` (per-DELIVERABLE, never a sibling's flag) and `validate` (an unknown version id is REFUSED, never defaulted into a generic bucket). The client mirror is `app.js reVariantEntries` — `telegramReReadyCount` carried the same binary and would have counted the repurpose copies for a version.

**Scene edits in a version:** ⧉ duplicate = **same image, a NEW movement prompt, therefore a new clip** (Hugo's definition) — zero image cost via the existing `api._apply_scene_duplicate` (every ready image cloned with its approval mirrored), one video credit per character when rendered by the explicit "▶ Rendera nya klipp" button. `+ bild → swappas` rides the run's OWN `POST /api/reengineer/{re_id}/scenes` with a `version_id` Form param rather than a parallel endpoint — that is why the person gate, Whisper prefill and QC come along unchanged. `📥 färdigt klipp` is modelled as a 📌 direct scene carrying `direct_clip_reused` so a full re-animate never renders over the user's own file. **`_finalize_reanimate` now writes `finals_stale` only when a BASE scene was touched** — rendering a version's clip used to tell Hugo his original reel was out of date; its READ stays unfiltered because it writes the whole scene list back and a filtered copy would delete the other cut. Endpoints: `POST/PATCH/DELETE /api/reengineer/{re_id}/versions[/{vid}]`, `.../{vid}/build`, `.../{vid}/animate`, `.../{vid}/rows/duplicate`, `.../{vid}/rows/import_clip`. UI: "🎞 Ny version" on the run card (gated on **`n_scenes`**, never `run.scenes` — the light history row omits it, the bug that hid the approval gate, the multi-person chooser and ↻ Kör om in turn) + a modal whose rows are keyed on a monotonic **uid** (index or scene_id stop being unique the moment a row can be duplicated, and Alpine then cross-wires the rows). The modal sits AFTER the Repurpose modal in `index.html` because `test_repurpose_telegram_toggle` slices the file between that modal's `x-show` and its button label. Both ✓ (mirror, auto-Telegram) ship PRE-TICKED, so a MISSING stored value resolves to True everywhere. NOT yet built: borrowing a scene + its finished clip from ANOTHER run, and versions for saved Editor reels. Locked by `test_versions.py` (33 cases; the cut filter and the dangling-row refusal each verified to FAIL with their fix reverted).

**🔁 Repurpose picks WHICH characters (2026-08-10, Hugo's directive).** A repurpose rebuilt the whole cast, always. The Repurpose modal now opens with a character list — "Alla" / "Ingen" plus a checkbox each, a 🔁 marker on anyone who already has a mirrored copy (it gets overwritten), and a count in the submit button. **A PER-CLICK filter, not a setting** (Hugo's call): every character opens TICKED on every open, so an unticked one can never carry silently into a later repurpose — `api._store_repurpose_settings` drops `char_ids` exactly as it already did, and `_repurposeBody()` sends the key ONLY for a real subset, since omitting it is precisely what "the whole cast" means server-side (an unfiltered click therefore keeps hitting the identical code path). Editor reels have no cast and never send the field; `openRepurposeModal`'s editor branch still initialises `chars`/`charIds` to `[]`, because `_repurposeBody()` reads them on every kind. Scope is the Repurpose modal ALONE — Step-6 compile and "Bygg ihop igen" are untouched. **The Swap path needed no runner work** (`/api/jobs/{id}/repurpose_videos` → `compile_job_videos(char_ids=…)` has taken the filter since the retry-one-character feature; the modal simply never sent it), so the new plumbing is all Reengineer: `runner_reengineer.repurpose(re_id, char_ids=…)` → `_do_repurpose`. **Two traps that filter opens, both closed here.** (1) `_do_repurpose` wrote `state["repurposed"]` as a WHOLE replacement, so a filtered build would DELETE every other character's earlier mirrored copy from the run card while its file sat on disk with nothing pointing at it — a filtered build now MERGES over the bucket re-read from DISK (not the snapshot taken at entry), letting the new entry win per character; the unfiltered path still replaces, so a stale entry for a character no longer in the run keeps disappearing. (2) Because it merges, the bucket then holds everyone, and `auto_finalize.send_reengineer_repurposed` would re-post the untouched copies to their channels on every partial run — it takes `char_ids` now, the same fix `send_reengineer_finals` got on 2026-07-31. `api._repurpose_char_ids` (shared by BOTH endpoints) refuses an empty or unknown pick LOUDLY with a 400 rather than letting it fall through to "finish Step 5 first" (Swap) or land `repurposing: False` with no card and no error (Reengineer); the client disables the button at zero too, and the toast + the `excluded` note are narrowed to the picked set so a deliberately unticked character is never reported as a problem with the build. **Delivery follows what was BUILT, never what was PICKED** — a picked character that `_char_is_uninvolved` skips is never rebuilt, and if an earlier repurpose left a `done` entry for it in the bucket, sending by the pick would re-post that stale copy to its channel. `_do_repurpose` therefore RETURNS the characters it wrote an entry for this pass and `repurpose()` delivers exactly those. An empty return must return BEFORE the send: `_send_reengineer_bucket` reads an empty `char_ids` as "no filter at all", so handing it one would re-post the run's entire existing bucket. **The ORIGINAL finals are untouched by construction and now asserted**: a repurpose writes only `repurposed` (never `finals`) and `repurpose_<cid>.mp4` beside `final_<cid>.mp4`; on the Swap side `_REPURPOSE_SLOT` shares no state field and no output filename with `_COMPILE_SLOT`. Nothing pinned either, so a refactor could have made the mirrored copy overwrite the real final with an upside-down reel as the only symptom. Locked by `test_repurpose_char_picker.py` (20 cases; the merge, the send filter, the built-not-picked rule and the empty-list guard each verified to FAIL when reverted) + `tests/js/repurpose_char_picker.mjs`.
...
**A retrying clip SAYS it is retrying (2026-08-10, Hugo: "hur vet jag att något retryar, står det fortfarande så här rött?").** The retry loop puts a refused clip straight back into `PROCESSING`, so on screen it was byte-identical to an ordinary slow render — the only evidence lived in the server log, and the one visible signal (the red content-policy panel) meant the opposite: gave up. `VideoVariant.refusal_takes` counts every refused take, reroute legs included, and is written BEFORE the next take starts — a counter written on the way out is missing for the whole ~60 s the user is watching. Serialized by `_job_to_dict`; app.js `clipRetryNote` paints an amber pulsing "⏳ nekad ×N — försöker igen (tagning N+1)" while the clip is `pending`/`processing`, and `clipTakesNote` leaves a grey "↻ gick igenom på tagning N+1" once it is `done`, which is what explains a clip that took five minutes. Both are deliberately SILENT on a failed clip: the red explainer panel already says the content check refused it, and two red messages about one clip is noise. Rendered in all three places a clip appears — Swap Step 5, the Reengineer clip strip, and the compact per-scene "Klipp:" row, where it REPLACES the plain ⏳ rather than sitting beside it. Locked by `test_clip_retry_visible.py` + `tests/js/clip_retry_note.mjs` (verified to fail against the pre-fix app.js), including a markup grep — a helper nothing calls is invisible, which is the exact bug.

**VERTEX AI is the third Veo host (2026-08-10) — same model, a quota you can actually raise.** The Gemini API's ceiling lifts only when a spend threshold is crossed ($100 + 3 days for Tier 2), which is circular: the quota is what stops the spending. Vertex serves the same `veo-3.1-fast-generate-preview` against a **per-project, per-region Google Cloud quota** — visible in the console, raised by a normal quota request. `clients/vertex_veo.py`, slug `veo-3.1-fast-vertex`, last in `VEO_HOST_ORDER`. **Auth is the one real difference**: Vertex rejects API keys outright (measured — "API keys are not supported by this API. Expected OAuth2 access token"), so the client mints a bearer token from a SERVICE-ACCOUNT JSON via `google.oauth2.service_account` (already an indirect dependency — no new SDK, and deliberately no `gcloud`, which is not installed here) and caches it for 50 min so a 40-clip batch mints once, not 40 times. Setup is `VERTEX_PROJECT_ID` + `VERTEX_LOCATION` + `VERTEX_CREDENTIALS_FILE` pointing at a key with the *Vertex AI User* role. **`veo_host_chain()` gates on `vertex_veo.configured()`, which requires the FILE to exist** — not just the setting to be non-empty — because a half-configured host would occupy the LAST leg and kill clips at token-mint time after everything else had already refused them. Unlike the Gemini path, Vertex accepts `generateAudio`, so a silent clip is expressible. **VERIFIED LIVE 2026-08-10** — submit, poll, inline-base64 download of an 8 s 1080×1920 clip with audio, and the RAI path all exercised against a real project. Two things the wire-up taught, both counter to what the docs implied: the model id is **`veo-3.1-fast-generate-001`**, NOT the `-generate-preview` the Gemini API answers to (probed across six candidates; `-preview` 404s on Vertex — do not "unify" the two constants), and **Vertex filters MORE strictly than the Gemini API on the same model** — Frank's shirtless frame, which the Gemini API rendered 5/5, was filtered on both Vertex attempts while a clothed frame rendered first try. So Vertex is real extra capacity for the clothed half of the cast and no help at all for the shirtless half, which is exactly why it sits LAST in the chain: it only ever sees clips the other two hosts already refused. A filtered video is not billed (Vertex says so in the refusal). Locked by `test_vertex_veo.py` (11 cases).

**THE VEO HOST CHAIN — fal once, then Google (2026-08-10, Hugo, explicitly TEMPORARY: "det här ska bara vara tillfälligt tills jag har mycket högre gränser på gemini api").** The two hosts of Veo 3.1 Fast differ only in who moderates and who rate-limits — same model, same speech fidelity, same 🎯 end-frame support. fal refuses 46% of these clips (measured over 127 identical frames) but has no practical daily ceiling; Google refuses almost none (5/5 on frames fal refuses 90-100% of the time) but accepted **~14 videos a day on Tier 1**, which cannot carry five 40-clip runs. So every clip takes **exactly ONE swing at fal** (`VEO_FAL_TAKES=1` — its refusals are image-driven, so a second identical take there is far less likely to help than moving on) and only what fal refuses spends Google's scarce capacity, where it is worth spending. Order lives in **`VEO_HOST_ORDER="fal,google"`**; setting it to `google` alone restores the 2026-08-10 measurement-driven default in one env var, which is the intended undo the day the Gemini limits are raised. `runner_media.veo_host_chain()` drops any host whose provider key is missing (a host we cannot reach would occupy a leg and turn one clear error into two), `veo_host_takes()` gives the LAST host the full `VIDEO_REFUSAL_RETRIES` budget since there is nowhere left to go, and `video_attempt_models` starts the chain **at the host the clip is already on** — a resumed or hand-retried clip must not spend a take back where it was already refused. `language_video_model()` replaces the bare `SPOKEN_LANGUAGE_VIDEO_MODEL` constant at every call site for the same reason. The CONTENT reroute chain (Kling, then Grok) still follows the whole host chain and is untouched by any of this. Locked by `test_google_veo.py`.

**A quota-blocked clip is PARKED and the run finishes itself hours later (2026-08-10, Hugo's choice).** Waiting inside the clip is not enough: measured on his Tier 1 key that day, Google accepted 3 clips at 03:22 UTC, refused everything until 07:03, took ~10 more, then refused everything again — **14 accepted submits against 46 refusals across the day**, so a 40-clip run cannot finish in one pass however patiently each clip waits. A clip whose ladder runs out is therefore marked `VideoVariant.quota_blocked` (STORED, never parsed back out of the Swedish error prose — the drainer must never pick up a clip that failed on content or network and re-bill it forever) and `runner.drain_quota_blocked` sweeps every `VEO_QUOTA_DRAIN_SECS` (default 900) until nothing is parked. The sweep is **sequential and stops at the first clip that is still blocked** — the window is shut or it is not, and walking 40 clips into a shut one is the hammering that re-arms the pause for everyone. It reads the store rather than memory, so a restart mid-wait does not strand a run. Started once per process from the app lifespan. Rejected alternatives: fal (that is where these clips get refused — the whole reason the path moved) and a manual ↻ per clip. Locked by `test_google_veo.py` (30 cases).

**A Veo quota wall is WAITED OUT, not routed around (2026-08-10, Hugo: "jag vill inte använda fal, fixa problemet hos google").** Google's Veo quota is per KEY and per TIER — on Tier 1 it ran out after a handful of clips and every remaining submit came back 429 `RESOURCE_EXHAUSTED`. Two separate defects showed up, both fixed: (1) each clip independently walked the client's backoff ladder — measured 162 s, 322 s and 486 s on a 9-clip batch, i.e. the run spent ~45 minutes learning the same fact nine times — so the first quota error now trips a shared pause (`google_veo._quota_block`, 600 s, EXPIRING rather than latching for the reason `fal_kling` documents about its own lock) and siblings fail in milliseconds; (2) the clip then simply failed, even though **the same model is available on fal**. the client now WAITS: `_RETRY_WAITS` runs 30/60/120/240/480/480 s (~25 min, bounded by the runner's 1 h video-phase timeout) and a sibling that meets the shared pause queues behind it instead of failing. **Waiting is right because the window measurably reopens** — the key that refused every submit at 03:45 UTC was accepting them again by 07:03 the same morning, same tier, same key. The fal host-fallback built first (`runner_media.video_host_fallback`, splicing the other host's attempts in at the current position) survives behind **`VEO_HOST_FALLBACK=1`, default OFF**: Hugo declined it because fal is where 46% of these clips get refused, which is the whole reason the path moved to Google, so a run that quietly finishes half its clips there is worse than one that waits. **A quota error is emphatically NOT a refusal**: `runner._is_quota_error` keys on the `GoogleVeoQuotaError` type, `is_content_rejection` does not match it, no `refusal_takes` are charged, and `video.fallback_model` is written at SUBMIT time from the model actually being submitted — a stale value there sends the resume poll to the wrong endpoint and 404s. With no fal key there is nowhere to go and the clip fails saying QUOTA, never pretending the content was blocked. Tier 2 (higher limits) needs $100 of cumulative spend plus 3 days, so this is Hugo's normal operating condition for now, not an edge case. Locked by `test_google_veo.py` (25 cases).

**Veo runs on GOOGLE'S OWN API, not fal (2026-08-10, Hugo's directive after the measurement below).** Hugo reported that prompts and images fal refused rendered "very often" in Google Flow. Four measurements over his own `calls.jsonl` + `state.sqlite3` identify the layer, and none of them is the model or the content: (1) **89% of fal's Veo refusals arrive AFTER a full render** — median 62.5 s against 64.7 s for a success, while Google's own pre-render refusal (`no_media_generated`) returns at 13 s; the video was built and discarded. (2) **Same account, same 127 start images, two models: Veo refused 128/281 (46%), Kling 16/198 (8%)**, and the two are UNCORRELATED (Kling refuses 7% of the images Veo hates and 13% of those Veo never touches) — a fal-wide checker would hit both alike, so this is the Veo path specifically. (3) **fal blames the prompt in 451 of 451 refusals** (`loc: ['body','prompt']`) while the real predictor is the IMAGE: within one character, bare-torso frames were refused 88-89% and fully clothed frames 0% (Susanne 16/18 vs 0/13, Helene 15/17 vs 0/23; 62 frames blind-scored by 12 agents that never saw a refusal rate). (4) **THE DECISIVE ONE — five frames fal refuses 90-100% of the time, two of which fal had NEVER once passed (0/9 and 0/8), rendered on Google's own API on the FIRST attempt** with nothing changed; under fal's own per-image pass rates that has probability ~1e-5. New client `clients/google_veo.py`, new slug **`veo-3.1-fast-google`**, and `SPOKEN_LANGUAGE_VIDEO_MODEL` points at it — which is "the default Veo path" in practice, since the 🗣 redirect is where nearly every Veo clip comes from. **A NEW slug, not a repointed one**: `_eff_video_model` resolves the polling host from the stored slug and fal request_ids are endpoint-scoped, so repointing `veo-3.1-fast` would have sent every in-flight clip's resume poll to the wrong provider. Everything asserted about the Google API was VERIFIED LIVE, not read off docs: `instances[0].lastFrame` interpolates start→end so the 🎯 end pose survives; `resolution: 1080p` gives 1080×1920 and `durationSeconds: 6` gives exactly 6.000 s, but the two TOGETHER are refused ("Your use case is currently not supported") — the same 1080p-needs-8s rule fal documents, so `_resolution` degrades identically; `generateAudio` is rejected as a field and audio (AAC stereo 48 kHz) comes anyway, so `generate_audio=False` cannot be honored and logs a warning instead of failing the clip; and the file download needs the key in the **`x-goog-api-key` header** — with `?key=` the endpoint answers 200 with an EMPTY body, which would have written a 0-byte clip that only fails later inside ffmpeg. **Google refuses too, and that is fine**: an end-to-end probe was filtered on take 1 and rendered on takes 2 and 3 of the identical request, so the refusal is stochastic there as well and the existing 5-take machinery covers it — the message now carries Google's own reason plus "You have not been charged for this attempt". **Quota is NOT a refusal**: the 4th concurrent submit returns 429, so submits are gated behind `GOOGLE_VEO_CONCURRENCY` (default 3) with backoff, and a genuine quota exhaustion raises `GoogleVeoQuotaError`, which `is_content_rejection` deliberately does NOT match — feeding it to the refusal machinery would spend the clip's whole take budget on a limit only time fixes. Cost: **$0.10/s at 720p, $0.12/s at 1080p, audio included**, against fal's $0.15/s with audio — an 8 s 1080p clip is $0.96 vs $1.20 — and Google states it only bills a successfully generated video while fal's FAQ warns a 422 may still be charged for GPU time, which every one of those ~450 post-render refusals spent. The fal slug stays registered as `Veo 3.1 Fast (fal)` for old jobs. Locked by `test_google_veo.py` (18 cases).

**A refused clip is REROUTED — Kling, then Grok (2026-08-10, Hugo: "för alla utom de tyska ska det automatiskt reroutas till andra modeller efter dessa försök, kling och sedan grok"). `VIDEO_MODERATION_FALLBACK=1` is now the DEFAULT.** After its own model has refused every unchanged take, a clip walks `runner_media.VIDEO_FALLBACK_CHAIN` — **one take per leg**, Kling 3.0 then Grok Imagine 1.5 — and only fails once every leg refused it. **GERMAN is the one exclusion and it is total** (Kling is measured 0.48 on German against 1.00 English / 0.93 Spanish, so a rescued German clip is one the wrong-language net has to reject anyway); those clips still fail loudly after their unchanged takes. Kling leads because a rescued clip has to fit the reel it lands in: it is in `END_FRAME_VIDEO_MODELS` so a 🎯 pose survives, it is the runs' own default model, and it refuses only 0-7% of its own calls against Veo's 32-79%. `video_fallback_chain()` filters the CHOSEN model out of the chain (it has already had its takes), so a clip picked on Kling walks only to Grok, and one picked on Grok walks only to Kling. `VIDEO_MODERATION_FALLBACK=0` is the kill switch for the WHOLE chain — including the Spanish Veo → Kling leg, which was flag-independent while it was a special case. **The 2026-08-03 reasons for turning this off are all still true and were accepted knowingly:** a rescued clip is rendered by a different provider than its neighbours, a Grok leg drops any resolved end pose (flagged via `drops_end_frame`, which is answered PER LEG — Veo → Kling keeps the pose, Kling → Grok loses it, and one clip can walk both), and a 🗣 clip leaves `SPOKEN_LANGUAGE_VIDEO_MODEL`. What changed is the refusal rate: Veo refused 79% of its calls on 08-08 and 70% on 08-09, so "fails loudly" had become the common outcome. Exhausting the chain fails naming the walked models with their take counts (`runner._chain_summary` → "veo-3.1-fast ×5 → kling-v3 → grok-imagine-1.5"), which is the signal to change the START FRAME. Locked by `test_video_moderation_fallback.py` (33 cases). **Historical (superseded by the paragraph above, kept for the measurements):** VIDEO moderation fallback → Grok Imagine 1.5 (2026-07-26; OPT-IN 2026-08-03 → 2026-08-10, `VIDEO_MODERATION_FALLBACK=1`, default OFF). Hugo 2026-08-03: "ta bort fallbacken till en annan modell om ett klipp failar". By default a clip its model refuses on content-policy grounds now **FAILS LOUDLY with the real reason** — no second provider is tried — so the user can reword or retry. Three reasons it was switched off: the rescue silently moved one clip in a reel onto a different provider (visibly and audibly unlike its neighbours), it dropped any resolved 🎯 end pose, and since the 🗣 language redirect it would have moved a German/Spanish clip off `SPOKEN_LANGUAGE_VIDEO_MODEL` — the one model trusted with that language. `runner_media.video_fallback_model()` is the single resolver both runners read (returns None when disabled). With the flag ON everything below applies unchanged. The video-side counterpart to the image ladder above. When a clip's chosen video model refuses it on CONTENT-POLICY / NSFW grounds, the runner re-submits the SAME clip ONCE on `grok-imagine-1.5` — a different provider stack (xAI via fal), markedly more permissive than Kling/Veo. Only GENUINE content rejections trigger it (`content_policy.is_content_rejection`); timeouts / network / fal-balance failures keep the normal fail path, and if the fallback ALSO refuses the clip fails LOUDLY naming the real reason (a non-content failure on the fallback leg reports its true cause, never a false content block). Single source of truth: `runner_media.VIDEO_MODERATION_FALLBACK_MODEL`. Covers every path that funnels through `runner._animate_one_video` (Swap synthesis, retry_one_video, generate_more_videos, retry_failed_videos, Reengineer animate/reanimate) plus the Reengineer direct/no-swap shared clip (`runner_reengineer._render_direct_clip`). Recorded on `VideoVariant.fallback_model` + a `video.fallback` event → violet ⇄ chip in the Swap Step-5 and Reengineer clip strips. `runner._eff_video_model` is fallback-aware so salvage re-poll and post-restart resume poll the provider the clip was ACTUALLY submitted under (fal request_ids are endpoint-scoped — polling Kling with a grok job id would 404). **END-FRAME CAVEAT:** `grok-imagine-1.5` is NOT in `END_FRAME_VIDEO_MODELS` and `pipeline.submit_video` never forwards `end_image` for it, so a clip with a resolved 🎯 end pose LOSES it on fallback. Hugo's decision: fall back anyway (a clip without the end pose beats no clip) but never silently — `runner_media.fallback_drops_end_frame(chosen)` detects the case, it's recorded on `VideoVariant.fallback_dropped_end_frame`, logged as a warning, and rendered as an amber "🎯✕ slutposen tappad" note beside the ⇄ chip. A scene whose model already ignored the pose is never flagged (false alarm). Originally built 2026-07-14 against `seedance-2.0` (end-frame-capable); that branch was deleted and the feature rebuilt on grok-imagine-1.5 on 2026-07-26. Locked by `test_video_moderation_fallback.py` (14 cases). **Scope narrowed 2026-08-04** — this generic rescue now applies to every clip EXCEPT a SPANISH `veo-3.1-fast` one, which has its own always-on rescue (next paragraph). A German or English Veo clip still falls under this generic, off-by-default rule.

**Refused SPANISH Veo clips → Kling 3.0 (2026-08-04; since 2026-08-10 simply the FIRST LEG of the chain above, same model and same order — the code no longer special-cases it, and `VIDEO_MODERATION_FALLBACK=0` now disables it too. The measurements below are unchanged and are why Kling leads the chain.) (Hugo: Hugo: "alla veo klipp som failar fallbackar till seedance 2.0 fast" — Seedance proved impossible, see below). ALWAYS ON — no env flag.** A `veo-3.1-fast` clip belonging to a 🇪🇸 character that fal REFUSES re-submits ONCE on `kling-v3`; every other clip keeps the 2026-08-03 default of failing loudly (see previous paragraph). Veo needed its own carve-out because it is where the 🗣 redirect sends EVERY Spanish clip and fal's checker kills them wholesale even at the least strict `safety_tolerance: 6` — measured on `j_619e0a2cf2`, **11 of 24** redirected Spanish clips (46%) died refused, on dialogue as innocuous as "Mezcla una cucharada de aceite de coco". Under the generic rescue those clips had nowhere to go: it is off by default, and `grok-imagine-1.5` would have undone the redirect. **WHY NOT SEEDANCE 2.0 FAST — do not retry this.** The feature was built on it first and it cannot work: ByteDance refuses EVERY frame containing a real person. Measured 2026-08-04 — **11 of 11** submits refused (7 in-app + 4 direct probes), on BOTH the fast and standard tiers, across three different faces, always `loc: [body, image_url]` / `content_policy_violation` / `partner_validation_failed` / "The images or videos provided may contain likenesses of real people…". A control frame with NO person rendered fine on the same endpoint and key, so it is ByteDance's real-people policy, not our account and not the prompt — and no prompt change can reach an image check. Every frame this app produces is a photoreal person, so Seedance can rescue exactly zero clips here. The `seedance-2.0-fast` slug added for it was removed again; `seedance-2.0` stays registered but has never rendered a clip in this app (0 successes, all-time). Guarded by `test_the_rescue_is_not_seedance`. **WHY KLING 3.0.** It is the only candidate both ACCEPTED and GOOD. On the clips Veo had actually refused, re-rendered verbatim: `kling-v3` 2/2 rendered, `grok-imagine-1.5` 2/2. Spanish speech fidelity over every Spanish clip on disk, scored the way video QC scores it (Scribe transcript vs the line the prompt asked for): **veo-3.1-fast n=37 mean 0.992** (min 0.909, 100% ≥0.8) · **kling-v3 n=106 mean 0.953** (min 0.000, 98% ≥0.8) · **grok-imagine-1.5 n=6 mean 1.000**. Kling costs ~0.04 mean against Veo and carries a ~1% English-leak tail (the one 0.000 clip spoke English on a Spanish prompt — the existing wrong-language net re-renders it). It beat Grok on the two things that make a rescued clip fit the reel it lands in: it IS in `END_FRAME_VIDEO_MODELS` so the 🎯 end pose survives, and it is the runs' own default model so the clip looks and sounds like its neighbours. Grok's 1.000 is 6 clips — too thin to outweigh either. **SPANISH ONLY** (Hugo's call): a refused GERMAN Veo clip still fails loudly, because Kling is measured 0.48 on German against 1.00 English / 0.93 Spanish — rescuing it there would ship a clip the language net has to reject anyway, slower and dearer for the same outcome. Resolver: `runner_media.video_fallback_model(chosen_model, language=…)` + `_veo_rescue_applies` — reads `VEO_VIDEO_MODELS` / `VEO_FALLBACK_LANGUAGES` / `VEO_MODERATION_FALLBACK_MODEL` and ignores `settings.video_moderation_fallback` for that pair; `fallback_drops_end_frame(model, language=…)` resolves the SAME way, so the Veo→Kling leg correctly reports no dropped pose while the generic→grok one still does. **Veo has a SECOND refusal shape** — fal `no_media_generated` ("The model did not generate the expected output for this prompt… including unsafe content"): it accepts the submit, runs, and returns nothing. 4 of the 11 refused Spanish clips failed this way and matched NO signal in `content_policy`, so they were dying with no rescue. `runner_media.triggers_fallback(model, exc)` treats it as a refusal ON THE VEO LEG ONLY — it is deliberately NOT added to `content_policy`'s global list, which also drives the image ladder and the generic rescue and whose other `no_media_generated` causes (incompatible media type, missing attachment) are real bugs that must keep failing loudly. Timeouts / network / fal-balance failures are never refusals anywhere. `_eff_video_model` checks `fallback_model` BEFORE `language_model_redirect`, so a rescued clip re-polls Kling's endpoint on salvage and post-restart resume (fal request_ids are endpoint-scoped — polling Veo with a Kling request_id 404s). If Kling ALSO refuses, the clip fails naming both. Locked by `test_video_moderation_fallback.py` (29 cases).

**A refused clip is RE-SUBMITTED UNCHANGED before anything else (2026-08-06, Hugo: "gör så fler videor inte blir nekade utan att ändra prompt"). `VIDEO_REFUSAL_RETRIES=4` (raised from 2 on 2026-08-10 — see the knob's note below for the measured rates).** The refusal is **not deterministic** — the identical prompt on the identical start frame passes on one take and is refused on the next — so the cheapest recovery is to send the same request again. `runner_media.video_attempt_models(model, language=…)` is the single source of truth both runners now build their attempt list from: `[chosen] * (1 + VIDEO_REFUSAL_RETRIES)` followed by the rescue model when one applies. **Order matters and was Hugo's call:** the extra Veo takes come BEFORE the Kling rescue, so a Spanish clip that passes on take 2 keeps Veo's 0.992 speech fidelity and still matches its neighbours in a 🗣 reel where every other clip is on Veo — Kling stays the last resort it always was. For GERMAN and ENGLISH clips, which have no rescue at all, this is the ONLY prompt-free recovery that exists. Wired into `runner._animate_one_video` (Swap synthesis + every retry path that funnels through it) and `runner_reengineer._render_direct_clip`. **A same-model take is NOT a fallback** — it must never set `VideoVariant.fallback_model`, which paints the ⇄ chip and, worse, is what `_eff_video_model` resolves salvage/resume polling against (fal request_ids are endpoint-scoped); both runners compare consecutive entries and only a model CHANGE takes the fallback branch, a repeat emits `video.refusal_retry` / `direct.clip.refusal_retry`. The trigger predicate is the existing `runner_media.triggers_fallback`, so ONLY genuine refusals repeat — a timeout, a network error or a locked fal account still fails on the first try (re-submitting a locked account is pure waste). Exhausting every take fails LOUDLY naming the count ("nekades 5 gånger av veo-3.1-fast"), because a clip refused on every take is a hard block on the INPUT — see the `no_media_generated` face/scene blocks below — and the fix there is a new start frame, not another ↻. **Measured basis (2026-08-06, over `calls.jsonl`):** Veo refused 42%/60%/53%/60% of its calls on 08-03…08-06, i.e. `safety_tolerance: 6` (which fal echoes in the payload from 08-03 onward, so it IS being sent) changed nothing — the `content_policy_violation` refusal takes 62.0 s median against 58.7 s for a success, so it fires AFTER the render, past the dial's reach, while `no_media_generated` returns in 13.0 s. **Billing:** fal prices Veo 3.1 Fast per GENERATED second ("for every second of video you generate you will be charged $0.10/$0.15"), and a refused take generates none, so the extra takes cost time, not credits. fal's general FAQ does warn that 422s "may still be charged if a runner spent GPU time" — if a fal invoice ever shows overhead against the count of FINISHED clips, set `VIDEO_REFUSAL_RETRIES=0`. Locked by `test_video_refusal_retry.py` (17 cases); `test_video_moderation_fallback.py` pins the knob to 0 so it keeps describing the rescue in isolation.

**Every Veo clip suppresses cuts, music and animal sounds (2026-08-10, Hugo's directive).** `clients/fal_veo._negative_prompt()` always LEADS the negative prompt with `camera cuts, background music, animal sounds`, then appends whatever `VEO_NEGATIVE_PROMPT` carries. **It is enforced in the CLIENT, not in the config default**, so setting `VEO_NEGATIVE_PROMPT` in `.env` can never silently drop the three terms — and it covers both fal endpoints (plain i2v and the 🎯 `first-last-frame-to-video` one) plus every path that reaches them: Swap synthesis, Reengineer animate/reanimate, per-clip ↻ retakes, the refusal re-submits above. Consequence: `VEO_NEGATIVE_PROMPT=` (empty) no longer omits the field and falls back to fal's own default — it sends the clause alone. Idempotent on the exact clause. **WORDING — deliberately NOT Hugo's spoken "No cuts, no music, no animal sounds.":** a negative prompt lists what to AVOID, so a leading "no" is a double negative, and the bare word `cuts` reads as skin wounds; `camera cuts` says the thing meant. **The POSITIVE prompt is untouched, and that is the point** — Hugo chose the negative field after we weighed placement, and it keeps this change clear of the dialogue extractor, the language nets, `_unparsed_dialogue_line`, video QC's expected speech, the (scene × gender × language) prompt cache and the ✎↻ prefill, i.e. every layer the 2026-08-02…08-09 silent-English bugs lived in. NOT applied to Kling: `KLING_NEGATIVE_PROMPT` is tuned for Kling's own failure modes and Hugo asked for Veo. NO measurement backs the terms yet — the existing `reengineer._NO_MUSIC_CLAUSE` (" No background music — natural ambient room sound only.", appended to every Reengineer scene prompt) is the positive-prompt counterpart and stays. Locked by `test_fal_veo.py` (8 cases, 5 verified to FAIL with the fix reverted).

**The character says the scripted line and nothing else (2026-08-10, Hugo: "ibland gör karaktären små ljud innan eller efter det den ska säga").** Every video model SYNTHESIZES the clip's audio from the prompt, so it pads the line with little vocal noises — hums, sighs, laughs, throat-clears, breaths — unless told not to. Exactly the defect class as the invented music bed, and like it there is no API switch: suppression is prompt-level. `reengineer._NO_ADLIB_CLAUSE` (" No ad-libbed sounds — no hums, sighs, laughs, throat-clearing, audible breaths or extra words before or after the scripted line.") is appended by the idempotent `reengineer.with_no_adlib`, from TWO call sites sharing that one string: `with_accent` (every Reengineer / Swap-from-images scene prompt, mirrored in the gate UI by app.js `klingSuffix`) and `runner._animate_one_video` (the universal backstop — plain Swap and 🎬 Animate prompts never pass through `with_accent` at all, a gap the e2e swap flow now pins). Applied AFTER the speaker fix and AFTER localization: it is an instruction, not speech, so it stays English like the pronounce / no-music clauses and the translator never sees it. **PROMPT-ONLY, by Hugo's choice** — no QC net, no re-renders. Note the existing gap that leaves: video QC's `speech_similarity` is SYMMETRIC, so a clip that says the line PLUS as many improvised words scores ~0.67, far above Hugo's 0.35 threshold — extra speech passes silently and nothing detects it if the clause is ignored. **The clause deliberately carries NO speech verb** (`says`/`speaks`/`spoken`/`voice` all match `_SPEECH_CONTEXT_RE`) and IS subtracted by `_without_own_clauses`: it is appended to SILENT shots too, and without both precautions a wordless B-roll prompt whose only quote is a product label would read as "orders speech, carries a line the extractor cannot parse" and be refused — the exact false refusal the 2026-08-08 `_NO_SPEECH_RE` fix removed. It carries no quotes, so `video_qc.expected_speech`, the caption script and every dialogue extractor read it as nothing. Also fixed here: app.js `klingSuffix`'s music branch added its clause to `suffix` but not to `out`, so every guard after it scanned text missing that clause — harmless while music was last, a trap the moment a clause follows it. Locked by `test_no_adlib_clause.py` (18 cases, 7 verified to FAIL with the fix reverted).
...
**A SWEDISH quote made every layer blind at once, and the net now asks the AUDIO (2026-08-09, Hugo's directive — "fixa så det aldrig händer i framtiden").** Hugo writes Swedish, where a quotation is `”så här”` — U+201D on BOTH sides. Every extractor pattern demanded `"` or `“` to OPEN, so `says …: ”Put baking soda on oranges…"` matched NOTHING: the localizer had no phrase to translate and rewrote only the accent clause, producing a prompt that ORDERED Spanish while STATING the line in English. Measured over the 1779 distinct prompts on disk, 52 carried a ≥5-word line no pattern could read and **every one of them opened with `”`**; widening the OPEN class to `["“”]` makes 56 readable and changes the extraction of ZERO prompts that already parsed (the BODIES are deliberately left alone — widening them to `„ «` truncated a real German CTA that quotes its keyword inline). **The structural half is why it was silent:** `_QUOTED_LINE_RE`, the net whose only job is to catch a line the extractor could not read, was written in the extractor's own alphabet, so one character class disarmed both layers together — the 2026-08-06 lesson recurring one level down. It now asks a shape-independent question: a long run between ANY two quote-like characters (`_QUOTE_CHARS = '"“”„«»'`; `‘’` stay out, `’` is the apostrophe in "isn't"). **A PAIR is a shape too** — two forms on disk have none, a line in SINGLE quotes (4 prompts) and a quote that opens and never closes (2), so `_unparsed_dialogue_line` gained two more probes, both carrying an apostrophe lookaround (504 prompts have one inside a word; without the guard, `can't reach you` opened a false "unterminated line" — 2 measured false refusals). Corpus-wide the three probes refuse 6 prompts, every one a genuine unreadable line, and prompts that order an accent while deliberately supplying NO words still render. **What was measured, not inferred:** in re_165ac37c1f (🇪🇸 wang + Ching, both male) 17 of 18 clips were saved only because the appended "speak ONLY Spanish" clause made Kling translate on the fly — **1 leaked English**, which is exactly the "lite engelskt tal" Hugo reported; on the same prompts in re_70f71c7204, before that clause existed, 8 of 9 spoke English outright. The 👥 speaker-fix agent accidentally rescued the FEMALE characters by rewriting their prompts with straight quotes, which is why only the male ones broke. **THE DURABLE NET — `video_qc.detect_spoken_language`:** every earlier check reads the PROMPT, so it is armed only when localization managed to read it and blind exactly when a shape defeats the extractor. This one classifies the TRANSCRIPT by stop-word frequency over {en, es, de} and needs no prompt at all, so no quote, verb or label can disarm it — and it is the only check that can catch a model ignoring a perfectly correct prompt (Kling leaks English on ~1 in 100 Spanish clips). `_transcribe` now runs for a 🗣 character even with NO expected line (that combination used to return None → `qc_status="skipped"`, 18 of 18 clips in re_165ac37c1f, indistinguishable from a silent shot). A hit routes through the existing `wrong_language` flag, so it inherits `VIDEO_QC_LANGUAGE_MAX_RETRIES` and the loud final failure — no new policy. Gated by `VIDEO_QC_LANGUAGE_MARGIN` (0.05): measured 0 false positives over 439 dialogue lines and 88 known-good finals, catching all 16 genuinely English clips in the affected runs plus the 4 of the documented 2026-08-06 leak; it abstains below 8 tokens, because "Un vaso de agua" is noise to any word-frequency method. **Nothing wrong-language reaches a final or Telegram** (Hugo: "stoppa båda"): `VideoVariant.wrong_language` is persisted, `_collect_clips`/`_assembly_gaps` report it as a HARD gap and `auto_finalize._all_videos_successful` refuses — while a merely GARBLED take still ships with its ⚠ chip, which is Hugo's deliberate 2026-07-03 flag-only setting, so the two are told apart by the stored flag and never by parsing the Swedish reason. The 409 now names the real reason per gap instead of calling a clip that rendered fine "missing". Also fixed here: `clip_failure` explained the refusal as "kolla OPENAI_API_KEY och kvoten" when nothing had been sent to the translator at all, and BOTH Reengineer final paths (assemble + 🔁 repurpose) never passed `language=` to the transcriber — `run_editor_pipeline`'s docstring claimed they did, 3ccc89c only touched `runner_compile.py`, and the measured cost is Scribe reading 3 of 20 German clips as Dutch or English, i.e. wrong-language captions over correct audio. Locked by `test_language_leak_2026_08_09.py`, `test_language_enforcement.py` and `test_assemble_coverage.py`, every new case verified to fail with its fix reverted.
...
**A missing end frame SAYS it is missing (2026-08-11, Hugo: "varför är det fortfarande vitt här").** Five blank white tiles under "Gemensam slutpose" were a SKELETON placeholder whose tooltip read "genererar slutbild…" — shown whenever a scene had a shared pose and a character had no swapped frame, **without ever asking whether anything was actually generating**. On re_bc2d243011 / j_edf16f0e4d the swap phase produced 30 variants and ZERO end frames (verified in `calls.jsonl`: exactly 30 `generate` calls, none for a pose), and `end_frame_errors` was empty — so the placeholder claimed progress indefinitely, offered nothing to click, and the run would have animated without the end poses it was configured for. Running `regen_scene_end_frames` by hand produced all five frames first try with no errors, so the machinery is sound; **why the swap-phase block never ran on that job could not be reconstructed from the surviving evidence** — the server log had been rotated by a restart, and every stored precondition (`end_frames_by_scene` populated, pose file present, scene not direct, characters carrying images, no early return in `_kick_char`) checks out after the fact. `reEndFrameGenerating(run)` now limits the skeleton to the states that actually produce poses (`swapping` / `awaiting_person_choice`), and `reEndFrameMissing(run, sc, jc)` renders an amber **"↻ saknas"** button in every other state that regenerates the scene's poses on click. An explicit error keeps its own red marker and a per-character uploaded frame still wins over both — one tile, one state. Locked by `test_end_frame_missing.py` + `tests/js/end_frame_missing.mjs`.
...
**Files are COPIED, never referenced across runs.** `DELETE /api/reengineer/{re_id}` rmtree's the whole run dir and no refcount anywhere is cross-run, so an inherited 🎯 pose or background pointing into the parent's dir would vanish SILENTLY (a missing pose is never an error — `_resolve_end_image` just returns None) and a referenced direct clip loudly-but-late (a waitable gap that never resolves). The shared 🎯 pose is read from `Job.end_frames_by_scene` — the live truth, since the post-gate set/regen/clear endpoints write only there while the scene entry's `end_frame_path` is write-once at upload — and copied into the child's dir. Scene images are the one safe reference: content-addressed, and nothing ever unlinks `input/scenes/`. **A scene id does not imply a file of that name (2026-08-08) — fixed at the source, `runner_reengineer.scene_path`.** Job scene paths were derived as `scenes_dir/<scene_id>.png`, hardcoded and unvalidated, in `_create_job_and_swap`. Two real populations are NOT at that path: a scene uploaded as **.webp** keeps its extension in `SceneAsset.filename` (2 in the library), and a **⧉-duplicated scene** (`…__dup<hex>`, 245 rows) gets a SceneAsset pointing at the SOURCE's file and never one of its own (`_apply_scene_duplicate` only inserts that path into `Job.scene_image_paths`). Both therefore got a path that does not exist — and since nothing validates it, EVERY variant for such a scene failed at generation time, per character, with no earlier signal. This predates the re-run feature and affected the normal upload path too. `scene_path` now resolves through the SceneAsset and falls back to the old derivation only when there is no usable row, so nothing that resolved before stops resolving. `rerun.canonical_scene_file` delegates to the SAME resolver — the modal must never offer a scene the job then cannot read — and `rerun.scene_file` adds two fallbacks (the parent job's `scene_image_paths`, then the base id a `__dup` chain was minted from) for a scene with no usable SceneAsset at all; only then is the scene re-registered under a fresh id backed by its own file, and only a genuinely unfindable image REFUSES the re-run loudly.
...
**The child is ALWAYS `from_images: True`**, even when cloned from a ♻️ video run: there is no source video to re-analyze, and `resume_all` dispatches on that flag — a False would send the child through the video path on restart and KeyError on `source_path`. Consequence: a re-run of a Reengineer run appears on the **Swap** tab, titled "↻ <parent name>" with `rerun_of` pointing back. **Button visibility:** `reCanRerun` falls back to `n_scenes`, NOT just `run.scenes` — the light history row (`GET /api/reengineer`) omits `scenes` and `loadReengineerHistory` hydrates only the newest 8 runs plus the gated ones, so gating on the array hid the button on every older FINISHED run (the third instance of that bug class, after the approval gate and the multi-person chooser). Locked by `test_rerun_button_visibility.py` + `tests/js/re_can_rerun.mjs`. Locked by `test_rerun_new_characters.py` (32 cases; the copy-not-reference, drop-runtime-keys and direct-reuse fixes each verified to FAIL when reverted).

**A ⧉-duplicated scene costs ONE image, not one per copy (2026-08-08, Hugo's directive).** Measured on j_94c3258684 (re_feba7996a8): scene `sc_ee6a1f88f7` had been duplicated four times, so five scene entries shared ONE frame and ONE prompt — and generated five DIFFERENT renders. Four wasted image generations per character (36 across that run's nine characters), and a reel whose "same" shot visibly jumps between clips. ⧉ duplicate is documented as costing zero image credits; the copies only actually shared the source's image when it already had a ready variant at duplication time, which is not the case when the run is (re)generated afterwards. `runner._kick_char` now groups slots by `_identical_generation_key` = (scene image **CONTENT** hash, prompt, take index) — the first renders, the rest reuse its file via `_mirror_duplicate_slots`. Content rather than path because `_apply_scene_duplicate` gives its copy the source's path while `register_scene_duplicate` writes a separate file with identical bytes; the PROMPT is in the key so a copy edited with 🪄 / ✎↻ still renders on its own. **The take index is load-bearing:** `images_per_character = N` renders N slots per scene from the identical prompt ON PURPOSE — they are the alternatives the gate offers — so take *i* of a copy mirrors take *i* of its leader and every copy still shows N distinct options (the first version of this fix collapsed them, caught by `tests/e2e/test_swap_flow.py`). Sharing the leader's file is the established shape: `_apply_scene_duplicate` already clones with `path=v.path`, `delete_variant` refcounts before unlinking, and `retry_single_variant` re-points a shared slot to its own path first, so ↻ on one copy still affects only that copy. A leader that fails or whose file vanished leaves its copies to render normally. Locked by `test_duplicate_scene_one_image.py` (11 cases). **This alone was NOT reliable (Hugo 2026-08-09, from the app, asking about a re-run of a run with a scene duplicated 4x): `_kick_char`'s dedup requires the PROMPT text to match too, and the reengineer AI Director writes free text per scene — measured on the reported run, the Director's own tool call simply OMITTED 3 of the 4 duplicate entries from its response, and they only ended up sharing a prompt because both fell back to the same job-wide stock string. That was an ACCIDENT of that one run, not a guarantee: a fresh Director call (a re-run always makes one) could just as easily write three DIFFERENT descriptions of the identical image, which `_kick_char`'s exact-text dedup would then treat as three separate requests.** Fixed at the source: `_create_job_and_swap` now dedupes scenes by image CONTENT **before** calling the Director (`runner._image_fingerprint`, first-seen wins) and copies the ONE answer onto every scene_id sharing that image, so the duplicate group has byte-identical prompt text BY CONSTRUCTION rather than by chance — `_kick_char`'s dedup then always matches. Cheaper too: fewer images in the Director's vision call for every run with duplicated scenes, not just a re-run. Verified against the reported run's actual scene files: 9 scene_ids, 6 distinct images, the 4-way duplicate collapses to ONE Director question. Locked by 3 cases in `test_reengineer_director.py`, including the exact omission failure mode.

**Character-background mode is ENFORCED, and QC can finally see it (2026-08-08, Hugo's directive).** In re_feba7996a8 six of nine images kept the SCENE's white American kitchen instead of the character's outdoor terrace — and **all nine passed image QC at `qc_attempts=1`**. Two independent defects, both measured before either was written. **(1) The model was asked to GUESS.** Character mode sent two references and hoped GPT Image 2 would infer the target environment from the identity photo. Blind-judged by 3 independent judges over 32 renders of the reported scene (majority vote, unanimous on 30 of 32): **4/16 correct** that way, **9/16** when the character photo is ALSO passed as the explicit background reference (Image 3) — the slot replacement mode already uses successfully. Fisher p=0.15, so suggestive rather than proven, which is precisely why (2) exists. Dispatch now duplicates the character photo into the Image 3 slot in character mode and appends `pipeline.BACKGROUND_LOCK`. Appended at DISPATCH for the same reason `CAST_LOCK` is: the failing runs use AI Director prompts, which are CUSTOM, so the mode-aware stock rebuild (`prompt in _stock`) never reaches them — measured, the failure is prompt-independent (43 stock-prompt renders fail the same way, so this was never a Director bug). The lock names Image 3, forbids the scene's PLACE from surviving *including through a doorway or window* (the most common half-failure: the kitchen intact with the character's hills swapped in outside), and deliberately does NOT tell the model to ignore the action reference — that image still supplies pose, framing and every held prop. An explicitly uploaded background still wins. **(2) QC was blind by construction.** The judge is told never to fail on a background — the deliberate 2026-06-30 catastrophe-only policy — so a kept scene passed every time. New conditional class **`BACKGROUND NOT REPLACED`**, armed ONLY by a `background_from_character` flag sent in character mode, exactly the shape of `PERSON COUNT` / `WRONG PERSON SWAPPED`; the blanket "a changed background never fails" bullet carries an explicit carve-out so the judge is never handed two contradictory rules. It costs no extra call or attachment (the character reference is already in the judge's context) and PASSES when the two places are indistinguishable. In `_REROLL_MARKERS` — a repair edit's "keep the background unchanged" contract is the exact opposite of the correction. **`SWAP_QC_BACKGROUND_MAX_RETRIES` (default 3) is independent of `SWAP_QC_MAX_RETRIES`, and this is the ONE class whose exhausted budget FAILS THE SLOT LOUDLY** instead of keeping the last take with a ⚠ chip (Hugo: "3 omförsök, och om det fortfarande är fel så faila högt") — an image in the wrong PLACE cannot be repaired downstream and would silently ship a broken reel. `0` degrades to flag-only. **REFUTED along the way, so nobody re-derives them:** the mode-blind "lit by the scene's own ordinary light" in `prompt_director.ORGANIC_STYLE_CLAUSE` (present in 101/101 scored prompts, 83 of which rendered CORRECTLY — a real defect with zero discriminating power); named kitchen props (neither sufficient nor necessary); the word "indoor" (a proxy for "the scene is an enclosed interior", not a lever); and the identity-first flip (correct, and applied identically to successes and failures). Locked by `test_character_background_enforced.py` (14 cases). **VERIFIED on the shipped path:** the 9/16 above was an early measurement using the STOCK replacement prompt WITHOUT the lock. Re-measured end to end on the exact dispatch that shipped — the real Director prompt from the failing run, `_dispatch_variant(..., background_mode="character")`, 16 renders against 16 on the old path, all 32 mixed and blind-judged by 3 fresh judges with **zero disagreement on any image**: **16/16 (100%) correct on the shipped path vs 4/16 (25%) on the old one, Fisher p<0.0001.** The Director's scene-specific detail plus the explicit Image 3 and the lock together do far better than the stock prompt did alone. One scene and one character, so the QC net remains the safety layer for the rest of the corpus.
...
**"Eget klipp" takes a DROPPED video, not only a picked one (2026-08-11, Hugo).** All three import controls — Swap Step 5 `📥 importera`, the Reengineer clip strip `📥 eget klipp`, the 🎞 versions modal `📥 färdigt klipp` — accept a drag-and-drop, because a user who learns the gesture in one strip will try it in the next. Two details carry the weight: **`dragover` is prevented ONLY for drags carrying files** (`onClipDragOver` reads `dataTransfer.types`), because the character library has its own internal drags — image reorder, drag-into-job — that carry custom types and no files, and a blanket `@dragover.prevent` would make the clip label look droppable for them and swallow the drop; this is the same rule `onImageReorderOver` already follows. And **a video whose mime type the browser did not recognise is accepted on its extension** — `.mkv`, and anything dragged out of certain apps, arrives with an EMPTY `file.type`, so refusing it would block real videos while the server validates the upload anyway. The picker and the drop share one uploader (`importSwapClipFile` / `importReClipFile`) rather than two copies that drift apart on the progress flag or the error handling. Locked by `test_clip_drop.py` + `tests/js/clip_drop.mjs` (verified to fail against the pre-fix app.js).
...
VIDEO_QC_LANGUAGE_MARGIN=0.05     # the SECOND wrong-language signal (2026-08-09), and
                                  # the durable one: `video_qc.detect_spoken_language`
                                  # classifies the TRANSCRIPT by stop-word frequency
                                  # over {en, es, de} and flags a 🗣 clip that came out
                                  # English. Needs no prompt at all, so no quote
                                  # character, verb or label can disarm it — unlike the
                                  # threshold above, which is armed only when
                                  # localization managed to READ the prompt. This is
                                  # the margin over the runner-up below which it
                                  # abstains. Measured 0 false positives over 439
                                  # lines + 88 known-good finals.
SWAP_QC_BACKGROUND_MAX_RETRIES=3   # re-rolls for the "BACKGROUND NOT REPLACED" class in
                                  # CHARACTER background mode only (2026-08-08).
                                  # Independent of SWAP_QC_MAX_RETRIES: an image
                                  # that kept the scene's room when the character's
                                  # own environment was ordered is unusable, and —
                                  # uniquely among QC classes — an exhausted budget
                                  # FAILS THE SLOT LOUDLY rather than keeping the
                                  # last take with a ⚠ chip. 0 = flag-only.
...
VEO_NEGATIVE_PROMPT="subtitles, captions, burned-in text, on-screen text, watermark, blur, distorted face, extra limbs"
                                  ...
                                  # earlier terms weigh more.
                                  # This is only the CONFIGURABLE TAIL: since
                                  # 2026-08-10 clients/fal_veo._negative_prompt()
                                  # always LEADS with "camera cuts, background
                                  # music, animal sounds" (Hugo's directive —
                                  # see the paragraph below), so setting this
                                  # in .env can't drop those three, and an
                                  # empty value now sends that clause alone
...
VIDEO_MODERATION_FALLBACK=1       # the REROUTE CHAIN for refused VIDEO clips
                                  # (2026-08-10, Hugo): ON = after its own
                                  # model refuses every unchanged take, a clip
                                  # walks Kling 3.0 → Grok Imagine 1.5, one
                                  # take each, and only then fails. GERMAN
                                  # clips are excluded in the resolver, not
                                  # here (Kling is 0.48 on German). 0 kills the
                                  # whole chain for every clip — including the
                                  # Spanish Veo → Kling leg, which used to
                                  # ignore this flag — so the run then behaves
                                  # the way German still does.
VIDEO_REFUSAL_RETRIES=4           # how many times a REFUSED clip is re-submitted
                                  # UNCHANGED on its OWN model before the rescue
                                  # (or the loud failure) takes over — Hugo
                                  # 2026-08-06, "fler videor ska inte bli nekade
                                  # utan att ändra prompt". The refusal is not
                                  # deterministic, so an identical request often
                                  # passes on take 2. Costs ~60 s per extra take
                                  # and nothing in credits (fal prices Veo per
                                  # GENERATED second; a refused take generates
                                  # none). Raised 2 → 4 on 2026-08-10 as Veo's
                                  # measured per-call refusal rate climbed to
                                  # 79% (08-08) / 70% (08-09): at ~70%, 3 takes
                                  # lose ~34% of clips and 5 takes lose ~17%.
                                  # 0 restores one take per model.
...
**The pad around speech is ASYMMETRIC (2026-08-10, Hugo's directive).** The single "Padding around speech" preset applied the same cushion to both sides of every cut; it is now two — **`pad_secs` = 0.1 s kept BEFORE speech resumes, `pad_end_secs` = 0.15 s kept AFTER it stops** (a phrase's tail needs more breathing room than its head). Split at the two pure keep-range builders, so every flow inherits it in one place: `video_edit._invert_silences` (level trim → `trim_silences` + `assemble_clips`, i.e. Editor Studio, the standalone Trim-silences card, Step-6 compile, Reengineer "Bygg ihop" and 🔁 repurpose) and `_word_gap_keep_ranges` (the opt-in word-gap trim). **`pad_end_secs=None` mirrors `pad_secs`** in every primitive, so a caller written before the split is never silently retuned. **The exact-start contract is untouched** (Hugo confirmed): the FIRST keep range still gets ZERO pre-pad, so a clip begins exactly on its own audio onset and hooks start where they always did — the pads govern the insides of pauses and the clip's tail, not its first frame. Preset carried by `CompileVideosBody` / `EditorRepurposeBody` / `ReAssembleSettingsBody` / the three editor Form endpoints / `ASSEMBLE_DEFAULTS` / `auto_finalize._DEFAULT_COMPILE_SETTINGS`, and exposed as two controls wherever there was one (Editor Studio ×2, Trim-silences card, Step-6 ⚙, Reengineer ⚙, Repurpose modal). NOT rewired: the word-gap trim's CALL SITES still pass no pad at all, so ticking "Ersätt-läge" keeps its own symmetric 0.05 — a pre-existing inconsistency, deliberately left rather than silently retuned. Locked by `test_pad_asymmetric.py` (13 cases, each verified to fail with its fix reverted).
