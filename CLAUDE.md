...
**Failed finals SELF-HEAL when the missing clip lands (2026-07-29, Hugo's directive).** The loud refusal above left a dead end: on `re_f786da400c` two clips failed during animate, the auto-assemble correctly refused those two characters' finals, the user retried the clips — and nothing rebuilt them, so every clip existed and two finals didn't. Now `runner.\_after_clip_done(job)` fires after EVERY clip lands DONE (generated, salvaged, resumed, imported) and, for `from_reengineer` jobs, calls `runner_reengineer.heal_failed_finals(job_id)`: a character whose final is `failed` (or absent) and whose `_collect_clips` coverage is now complete gets rebuilt via `assemble(re_id, only={cid})`. `_do_assemble(..., only=…)` builds just those characters and MERGES into the run's existing finals (re-read at write time), so a finished final is never re-rendered, never re-billed, and `finals_stale` is left alone on a partial build; `auto_finalize.send_reengineer_finals(re_id, only=…)` likewise delivers ONLY the healed characters to Telegram (still gated on `auto_telegram_send` + the run landing `done`). No-op before the first build (empty `finals` → the video watcher owns it), while a clip is still rendering, or while `_ASSEMBLING`/`_HEALING` holds the run. The shared "direct / no swap" clip heals too (`_render_direct_clip` calls the same helper on success). Locked by `test_final_selfheal.py`.
