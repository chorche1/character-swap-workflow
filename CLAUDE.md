...
**Both pickers in the ↻ re-run modal too (2026-08-09, Hugo's directive).** The "↻ Kör om med nya karaktärer" modal already SENT `character_source_image_ids` and `rerun.build_state` already honoured it — but nothing in the UI could set it, and the character chips rendered `swapCharThumb()`, i.e. whatever reference image happened to be staged on the always-visible Swap upload card. So the modal showed a picture it was not going to use and offered no way to change it. It now carries the same two controls as the upload forms — the per-character "N ↕" popover (`rerunCharThumb` / `pickRerunSource`, writing the modal's OWN `rerunModal.sourceOverrides`) and the bulk "Använd bild för alla" dropdown (`bulkSourceForm('rerun')`, which is why that resolver is now a three-way branch and not a fall-through to the Swap form — a 'rerun' kind reaching it would have written the wrong map against the wrong selection, silently). Opening the modal clears the picks, the note and any open popover: a pick carried into the NEXT re-run would swap from the wrong reference with nothing on screen saying so. Every other rule is inherited unchanged (clamp-to-last, ★ primary = no override, bulk-then-individual, note dropped on a cast change). Locked by `test_rerun_source_image.py` (+ its `tests/js/rerun_source_image.mjs` harness).
