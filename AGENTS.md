...
**Push-to-Drive (2026-07-02, Hugo's directive).** One-click upload of finished videos to Google Drive, replacing the deleted Drive *inbox* with a *push* direction. Folder layout (auto-created under the `drive.file` scope — the app only ever sees folders it created): `Character Swap/<character name>/` receives Swap Step-6 compiled finals + Reengineer finals (and their 🔁 repurposed variants), `Character Swap/Editor/` receives Editor finals + repurposed reels. **Re-push OVERWRITES the same Drive file** (same name in the same folder — Drive keeps its own version history); filenames embed the owning job/run id (`"<title> — <char> [<id>].mp4"`) so a rebuilt final re-pushes over itself while other runs' files coexist. Endpoints: `POST /api/jobs/{job_id}/characters/{char_id}/drive_push` + `POST /api/reengineer/{re_id}/chars/{char_id}/drive_push` (body `{variant: "final"|"repurpose"}`), and `POST /api/editor/{edit_id}/drive_export` now defaults to the shared Editor folder (+overwrite) when no `folder_id` is given — an explicit `folder_id` keeps the old create-only behavior; optional `gen_id`+`slot` persist the receipt on a saved reel. Receipts persist on `JobCharacter.drive_pushes[variant]`, `state[finals|repurposed][cid]["drive"]`, and `editor_meta["drive"][slot]` so the ✓ survives reloads. UI: ⬆ Drive buttons on the Step-6 compile cards, both repurpose cards, Reengineer final cards, and saved-reel cards; a 409 "not authorized" auto-runs the one-time `drive.file` OAuth bootstrap and retries. Failures are LOUD (toast with the real reason). Helpers: `google_drive.ensure_folder_path` / `upload_or_replace` / `DriveNotAuthorized`; `api._drive_push_file`. Locked by `test_drive_push.py`.
...
```
ANTHROPIC_API_KEY=...             # 🎬 AI Director (Codex Opus with vision; toggle on
...
CLAUDE_OPUS_MODEL=Codex-opus-4-5        # AI Director — override to roll forward to a newer Opus
...
├── prompt_director.py — Heavy (🎬) AI Director: Codex Opus vision + tool-use writes
...
Opt-in Codex/Opus agent that writes tailored per-variant prompts. Toggle (🎬) sits next to ✨ enrich on the Swap Step-2 form (+ the Reengineer upload checkbox — see the Reengineer AI Director paragraph above). Disabled when `ANTHROPIC_API_KEY` isn't set; UI greys out the checkbox + shows a tooltip.

### What it does

- **Swap** — ONE Codex Opus call before image gen. Sees every character image + every scene image with vision. Uses tool-use (`submit_swap_plan`) to return a complete plan: a tailored prompt per (character × scene × variant_index). Plan is cached as JSON on `Job.director_prompts_json` so retries / resumes don't re-bill. `runner._kick_char` reads the cache and assigns each `GeneratedImage.prompt` from `plan.lookup(char_id, scene_id)[variant_idx]`. Falls back to enrich → raw → `GENERATION_PROMPT` per slot if the plan is missing or fails.
- **Swap movement (Step 4)** — second Codex call with the scene image + every approved variant image + the user's per-scene movement text. Returns one cinematic shot prompt per scene; merged into `enriched_movement_prompts` so the existing per-scene resolver in `run_video_synthesis` transparently picks it up.
