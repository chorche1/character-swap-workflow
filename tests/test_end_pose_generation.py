...
The user sets an optional end-POSE per scene in Step 1; at job creation it lands
on Job.end_frames_by_scene; during Step 3 (`_kick_char`, alongside the variants)
the runner swaps the character into that pose and stores the result on
JobCharacter.end_frame_paths[scene_id]. A failed swap is RECORDED on
end_frame_errors (surfaced, never swallowed — that silent swallow was why the
first version of this feature was reverted). duplicate_scene carries both over.

Hermetic: create_job's CreateJobBody resolution is tested via a stub store; the
runner's end-frame generation is tested by stubbing pipeline.generate_variant so
no real image API is hit.
"""
...
def _kick_job(tmp_path, pose: Path) -> tuple[Job, JobCharacter]:
    ...
    job = Job(
        job_id="j_ef", title="t", scene_id="s1",
        scene_image_path=str(tmp_path / "s1.png"),
...
@pytest.fixture
def _runner_stubs(monkeypatch, tmp_path):
    """Stub persistence/emit so the runner runs without a real store/event loop."""
...
def test_kick_char_generates_end_frame(tmp_path, monkeypatch, _runner_stubs):
    # Stub image generation: variants + end frame both just touch their dest.
    ...
    _run(runner._kick_char(job, jc, 1, asyncio.Semaphore(2)))

    # The end frame for scene s1 got generated + recorded; no error.
    assert "s1" in jc.end_frame_paths
    assert Path(jc.end_frame_paths["s1"]).exists()
    assert "s1" not in jc.end_frame_errors


def test_kick_char_surfaces_end_frame_error(tmp_path, monkeypatch, _runner_stubs):
    """REGRESSION: a failed end-frame swap must land on end_frame_errors, never
    be silently swallowed (the bare-except swallow was why v1 was reverted)."""
    pose = tmp_path / "pose.png"; pose.write_bytes(b"pose")

    def fake_generate_variant(*, scene_image, dest, **kw):
        # Normal variants (scene image) succeed; the end-frame swap (pose as the
        # scene ref) is rejected on content policy and stays unrecoverable.
        if str(scene_image) == str(pose):
            raise RuntimeError("content_policy: sexual content blocked")
        Path(dest).parent.mkdir(parents=True, exist_ok=True)
        Path(dest).write_bytes(b"img")
        return Path(dest)
    monkeypatch.setattr(runner.pipeline, "generate_variant", fake_generate_variant)
    ...
    assert "s1" in jc.end_frame_errors
    assert "content_policy" in jc.end_frame_errors["s1"]
