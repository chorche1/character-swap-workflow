...
async def _animate_one_video(
    ...
    if lang_spec is not None:
        try:
            ...
            # already-translated line verbatim). Folded into the cache key
            # too — a forced re-localize of an override must never reuse a
            # scene-mate's un-forced cache slot for the same literal text.
            force_relocalize = bool(video.movement_prompt_override)
            key = _prompt_cache_key(job.job_id, "localize", approved.scene_id,
                                    char_gender, lang_spec.code,
                                    movement_prompt
                                    + ("\x00force" if force_relocalize else ""))
            localized = await _cached_prompt(key, lambda: asyncio.to_thread(
                reengineer.localize_motion_prompt, movement_prompt,
                lang_spec.code, job_id=job.job_id, force=force_relocalize))
