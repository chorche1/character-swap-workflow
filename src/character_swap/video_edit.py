...
TEMPLATES: dict[str, CaptionStyle] = {
    ...
    # Size 56 is Hugo's caption-size standard (2026-07-04, was 60) — the
    # editor-wide preset applied across ALL flows: Swap Step-6 compile + the
    # standalone Editor tab read this template size directly (no override),
    # and Reengineer/Repurpose ride a matching size=56 caption override.
    "capcut-bluebox": CaptionStyle(font="Poppins", size=56,
...
def assemble_clips(video_paths: list[Path], output_path: Path, *,
    ...
    with record(phase="editor_assemble_clips", model="ffmpeg-single-encode",
        ...
        for ci, (p, has_audio) in enumerate(zip(video_paths, has_audio_flags)):
            ...
            # too little CUTTABLE silence (pauses ≥ min_silence_secs —
            ...
            if enable_interior_trim:
                ...
                def _cuttable(sil: list[tuple[float, float]]) -> float:
                    return sum(e - s for s, e in sil
                               if (e - s) >= min_silence_secs)

                need = min(1.5, max(0.35, 0.05 * duration))
                esc = thr
                while esc < ceiling and _cuttable(silences) < need:
