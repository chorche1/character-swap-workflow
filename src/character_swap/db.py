...
def ensure_schema(conn: sqlite3.Connection) -> None:
    ...
    # Per-character Telegram channel — where this character's finished videos
    # are auto-delivered. Was model-only (2026-07-28): the field existed on
    # CharacterAsset and the PATCH endpoint wrote it, but nothing persisted it,
    # so every restart silently blanked the channel and the auto-send reported
...
def load_app_state(conn: sqlite3.Connection) -> AppState:
    ...
    # NB: a `chats` table from the removed Chat tab may still exist in live
    # SQLite files — it is simply never read or written anymore.
...
def reset_all(conn: sqlite3.Connection) -> None:
    for table in ("gen_reference_paths", "generations",
        ...
        # Tolerate tables missing on older DBs.
        try:
