...
class Job(BaseModel):
    ...
    # NOTE: only end-frame-capable models (kling-v3, seedance-2.0, veo-3.1-fast
    # — see runner_media.END_FRAME_VIDEO_MODELS) honor per-scene END FRAMES; a
    ...
    # result to an end-frame-capable model (Kling 3.0 / Seedance 2.0 / Veo 3.1
    # Fast) as the end frame — first/last-frame interpolation. Keyed by scene_id, so a
...
class GenKind(StrEnum):
    IMAGE = "image"
    VIDEO = "video"
    AVATAR = "avatar"   # legacy kinds — rows must still load (Avatar/Audio tabs
    AUDIO = "audio"     # removed; old MediaGeneration rows keep these kinds)
...
class AppState(BaseModel):
    # NB: old state.json files may still contain a "chats" key from the removed
    # Chat tab — pydantic's default extra="ignore" silently drops it on load.
