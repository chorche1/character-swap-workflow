...
async def _watch_video_phase(re_id: str, job_id: str, *,
    ...
    if not _auto_chain_opted_in(state):
        ...
        return
    # Whichever trigger got us here (⚡ auto_mode or the image-sourced
    # auto_telegram_send chain above), build ONLY when every character's every
    # clip actually SUCCEEDED (Hugo 2026-07-29). The loop above waits for
    # TERMINAL clips — done OR failed — so a single failed clip used to trigger
    # the auto-build anyway and the character failed loudly inside it. Park at
    # the manual gate with the reason instead; the user retakes the clip and
    # presses ▶ Bygg ihop.
